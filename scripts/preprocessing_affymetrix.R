...
# install.packages("http://mbni.org/customcdf/22.0.0/entrezg.download/hgu133plus2hsentrezgcdf_22.0.0.tar.gz", type="source", repos=NULL)
# install.packages("http://mbni.org/customcdf/22.0.0/entrezg.download/hgu133plus2hsentrezgprobe_22.0.0.tar.gz", type="source", repos=NULL)
# install.packages("http://mbni.org/customcdf/22.0.0/entrezg.download/hgu133plus2hsentrezg.db_22.0.0.tar.gz", type="source", repos=NULL)
...
gds <- getGEO("GSE122214")

pd = pData(gds$GSE122214_series_matrix.txt.gz)
pd = pd[which(pd$`condition:ch1`== "first trimester from natural pregnancy"),]
nrow(pd)
...
files = list.files("raws/affymetrix/GSE122214_RAW/cell/")
...
affyData = ReadAffy(phenoData=pd,
                    sampleNames=pd$geo_accession,
                    filenames=pd$file_name,
                    celfile.path="raws/affymetrix/GSE122214_RAW/cell/")

colnames(exprs(affyData)) == pd$geo_accession
colnames(exprs(affyData))
...
ncol(exprs(affyData.rma))
nrow(pd)
ncol(affyData.rma)

exprs = exprs(affyData.rma)
...
write.table(pd, paste(prepath, '/', studies$accession[[i]], "_preprocessed_affymetrix_pdata_control.tsv", sep=""), sep="\t", quote=FALSE)
...
pca = prcomp(t(as.matrix(sub_exprs)))


pl = fviz_pca_biplot(pca, axes = c(1,2),
                  label=c("var"),
                  habillage=sub_pdata$Comment..Sample_source_name.,
                  repel = TRUE,
                  title = "",
                  # palette = "Set3",
                  # addEllipses = TRUE,
...
pl = fviz_pca_ind(pca, axes = c(1,2),
                  ...
                  palette = "Set4",
                  # addEllipses = TRUE,
                  col.ind = sub_pdata$Comment..Sample_source_name.
...
write.table(exprs, paste(mappedpath, '/', studies$secondary_accession[[i]], "_mapped_affymetrix.tsv", sep=""), sep="\t", quote=FALSE)

paste(mappedpath, '/', studies$accession[[i]], "_mapped_affymetrix.tsv", sep="")
...
pl <- pcaPlots(pca, pd, c("type"), ncol=2)
...
eset = ExpressionSet(as.matrix(exprs))
eset@phenoData = AnnotatedDataFrame(pd)
pd$Comment..Sample_source_name.
arrayQualityMetrics::arrayQualityMetrics(
  eset,
  outdir = paste(plotsqcpath,studies$accession[[i]],"1", sep='/'),
  force = TRUE,
  intgroup = 'Comment..Sample_source_name.'
)
